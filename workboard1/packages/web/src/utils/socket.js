...
export const initializeSocket = () => {
  ...
  // Handle task updated events (different from task:update)
...
// FIX: Update task broadcast to match server expectations
export const broadcastTaskUpdate = (projectId, task) => {
  if (socket && socket.connected) {
...
// FIX: Chat function - changed event name to match server
export const sendMessage = (projectId, message) => {
  if (socket && socket.connected) {
    // Changed from 'message:send' to 'chat:message' to match server
    socket.emit('chat:message', { projectId, message });
    console.log('Sending message:', { projectId, message });
  } else {
